...
const NavButton: FC<NavButtonProps> = ({ x, y, label, link, icon, newTab }) => {
  ...
  return (
    <ResponsiveComponent>
      {({ size }: ResponsiveProps) => {
        const isWide = typeof size === 'number' && size >= 480
        ...
        return isWide ? (
          ...
          >
            ...
            >
              <span
                className="relative w-14 h-14 p-4 hover:text-[rgb(var(--accent))]"
                style={{ animation: 'spin-reverse 100s linear infinite' }}
              >
                {getIcon(icon)}

                <span className="peer bg-[rgb(var(--transparent))] absolute top-0 w-full h-full" />

                <span className="absolute hidden peer-hover:block px-2 py-1 left-full mx-2 top-1/2 -translate-y-1/2 bg-[rgb(var(--background))] text-[rgb(var(--foreground))] text-sm rounded-md shadow-lg whitespace-nowrap">
                  {label}
                </span>
              </span>
            </NavLink>
